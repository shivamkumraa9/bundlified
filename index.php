...
if ($decodedToken) {
    ...
    try {
      ...
      // echo $productsJson;
      // Since the response is already an array, assign it directly
  
      $filteredProducts = $products['filtered']; // Products without the metafield or with value 0
      $excludedProducts = $products['excluded'];       

      // Extract product titles from the products array
...
<?php endforeach; ?>
    ...
    const ulElement = document.querySelector('.Polaris-List'); // Get the <ul> element
    const newListItem = document.createElement('li'); // Create a new <li> element
    newListItem.className = 'Polaris-List__Item'; // Add the required class
    newListItem.textContent = content; // Set the text content or HTML
    ulElement.appendChild(newListItem); // Append the new <li> to the <ul>
}
    ...
    async function deleteSelectedOption() {
        ...
        // Check if the same product is selected in both dropdowns
        ...
        try {
            ...
            const response = await fetch('createBundle.php', {
                method: 'POST',
                headers: {
        ...
        // Fetch the filtered and excluded products from the backend
        const filteredProducts = <?php echo json_encode($filteredProducts); ?>;
        const excludedProducts = <?php echo json_encode($excludedProducts); ?>;
        console.log(filteredProducts, excludedProducts)

    // Add event listener to update the selected option text whenever the selection changes
